...
OUTPUT_ROOT = os.path.join(TASK1_ROOT, "Task1_sentiment_evaluation")

FINAL_EXCEL_PATH = os.path.join(OUTPUT_ROOT, "Task1_sentiment_evaluation.xlsx")
...
def clean_and_merge(output_csv: str, df_answer_cleaned: pd.DataFrame) -> pd.DataFrame:
    ...
    sa = df_compare['NegPos_answer'].astype(str).str.strip().str.lower()
    so = df_compare['NegPos_output'].astype(str).str.strip().str.lower()

    df_compare['sentiment_match'] = sa == so

    df_compare['Matrix'] = ""
    df_compare.loc[(sa == "") & (so == ""), 'Matrix'] = "TN"
    df_compare.loc[(sa != "") & (so != "") & (sa == so), 'Matrix'] = "TP"
    df_compare.loc[(sa != "") & (so == ""), 'Matrix'] = "FN"
    df_compare.loc[(sa != "") & (so != "") & (sa != so), 'Matrix'] = "FN"
    df_compare.loc[(sa == "") & (so != ""), 'Matrix'] = "FP"
...
def evaluate_one_model(model_dir: str, df_answer_cleaned: pd.DataFrame):
    ...
    for shot in SHOT_FOLDERS:
        ...
        for filename in sorted(os.listdir(csv_dir)):
            ...
            out_name = f"{run_name}_sentiment_eval.csv"
    ...
    df_results.to_csv(os.path.join(model_output_root, "sentiment_eval_per_run.csv"), index=False)
    df_shot_avg.to_csv(os.path.join(model_output_root, "sentiment_eval_per_shot.csv"), index=False)
...
def evaluate_all_models():
    ...
    for name in sorted(os.listdir(TASK1_ROOT)):
        ...
        if os.path.isdir(full_path) and name.startswith("Task1_") and name != "Task1_sentiment_evaluation":
    ...
    with pd.ExcelWriter(FINAL_EXCEL_PATH, engine="openpyxl") as writer:
        df_all_per_run.to_excel(writer, sheet_name="ALL_sentiment_eval_per_run", index=False)
        df_all_per_shot.to_excel(writer, sheet_name="ALL_sentiment_eval_per_shot", index=False)

    df_all_per_run.to_csv(os.path.join(OUTPUT_ROOT, "ALL_sentiment_eval_per_run.csv"), index=False)
    df_all_per_shot.to_csv(os.path.join(OUTPUT_ROOT, "ALL_sentiment_eval_per_shot.csv"), index=False)
