...
OUTPUT_ROOT = os.path.join(TASK1_ROOT, "Task1_topic_evaluation")

FINAL_EXCEL_PATH = os.path.join(OUTPUT_ROOT, "Task1_topic_evaluation.xlsx")
...
def clean_and_merge(output_csv: str, df_answer_cleaned: pd.DataFrame) -> pd.DataFrame:
    ...
    df_compare.loc[~has_answer, 'NegPos_answer'] = ""
    df_compare.loc[~has_output, 'NegPos_output'] = ""

    df_compare['Topic_Match'] = (has_answer & has_output) | (~has_answer & ~has_output)

    match = df_compare['Topic_Match'] == True
    df_compare['Matrix'] = ""
    df_compare.loc[match & has_answer & has_output, 'Matrix'] = "TP"
    df_compare.loc[match & ~has_answer & ~has_output, 'Matrix'] = "TN"
    df_compare.loc[(~match) & ~has_answer & has_output, 'Matrix'] = "FP"
    df_compare.loc[(~match) & has_answer & ~has_output, 'Matrix'] = "FN"
...
def evaluate_one_model(model_dir: str, df_answer_cleaned: pd.DataFrame):
    ...
    for shot in SHOT_FOLDERS:
        ...
        for filename in sorted(os.listdir(csv_dir)):
            ...
            out_name = f"{run_name}_topic_eval.csv"
    ...
    df_results.to_csv(os.path.join(model_output_root, "topic_eval_per_run.csv"), index=False)
    df_shot_avg.to_csv(os.path.join(model_output_root, "topic_eval_per_shot.csv"), index=False)
...
def evaluate_all_models():
    ...
    for name in sorted(os.listdir(TASK1_ROOT)):
        ...
        if os.path.isdir(full_path) and name.startswith("Task1_") and name != "Task1_topic_evaluation":
    ...
    with pd.ExcelWriter(FINAL_EXCEL_PATH, engine="openpyxl") as writer:
        df_all_per_run.to_excel(writer, sheet_name="ALL_topic_eval_per_run", index=False)
        df_all_per_shot.to_excel(writer, sheet_name="ALL_topic_eval_per_shot", index=False)

    df_all_per_run.to_csv(os.path.join(OUTPUT_ROOT, "ALL_topic_eval_per_run.csv"), index=False)
    df_all_per_shot.to_csv(os.path.join(OUTPUT_ROOT, "ALL_topic_eval_per_shot.csv"), index=False)
