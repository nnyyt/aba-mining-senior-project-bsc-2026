...
MODEL_TAG = "deepseek-r1:7b" #can change to model to gemma-7b or qwen-7b or something else if you have it running in Ollama
...
OUTPUT_BASE_DIR = "/Users/nonny/Downloads/Senior Project 2025/Task3-Voted Dataset/Deepseek"
...
RUN_NAME = f"task3_contrary_{SAFE_TAG}{SHOT_SUFFIX}"
...
def get_prompt_0shot(phrase_a: str, phrase_b: str) -> str:
    return (
        ...
        "Don't provide any nonsensical or multilingual garbage text.\n\n"
...
def load_done_keys(run_csv_path: str) -> set[str]:
    ...
    try:
        ...
        return {
            make_key(rid, a, b)
            for rid, a, b in zip(df_old["ID"], df_old["PhraseA"], df_old["PhraseB"])
        }
    except Exception:
        return set()
...
# ========================= Per-sheet runner =======================
def run_one_sheet(sheet_name: str, input_xlsx: str, out_root: str, run_name: str) -> pd.DataFrame:
    ...
    run_base = f"{run_name}_{sheet_token}"

    master_xlsx = os.path.join(sheet_dir, f"{run_base}_sheet.xlsx")
...
def run_one_workbook(input_xlsx: str, out_root: str, run_name: str):
    os.makedirs(out_root, exist_ok=True)
    all_sheets_xlsx = os.path.join(out_root, f"{run_name}_ALL_sheets.xlsx")
    ...
    for s in targets:
        try:
            print(f"\n[RUN] Sheet: {s}")
            all_outputs[s] = run_one_sheet(s, input_xlsx, out_root, run_name)
        except Exception as e:
...
def main():
    ...
    for fname in INPUT_FILES:
        ...
        out_root = os.path.join(OUTPUT_BASE_DIR, f"{RUN_NAME}_{tag}")

        print("\n" + "=" * 80)
        print(f"[FILE] {fname}")
        print(f"[IN  ] {input_xlsx}")
        print(f"[OUT ] {out_root}")
        print("=" * 80)

        run_one_workbook(input_xlsx, out_root, RUN_NAME)
