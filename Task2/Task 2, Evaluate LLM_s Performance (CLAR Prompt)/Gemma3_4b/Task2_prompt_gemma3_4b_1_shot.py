...
def make_pair(rid, claim) -> str:
    return f"{str(rid).strip()}||{norm(claim)}"

def enforce_run_cols(df: pd.DataFrame, run: int) -> pd.DataFrame:
    col_test = f"Test {run}"
...
def enforce_log_cols(df: pd.DataFrame) -> pd.DataFrame:
    cols = ["ID", "Claim", "Output"]
    for c in cols:
        if c not in df.columns:
            df[c] = ""
    return df[cols]

def normalize_existing_run_csv(run_csv: str, run: int):
    """Rewrite existing run CSV with correct header order + dedupe by ID+Topic."""
    if not os.path.exists(run_csv):
        return
    try:
        df = pd.read_csv(run_csv, dtype=str)
        df = enforce_run_cols(df, run)
        df = df.drop_duplicates(subset=["ID","Topic"], keep="last")
        df.to_csv(run_csv, index=False)
    except Exception:
        pass

def normalize_existing_log_csv(run_log: str):
    """Rewrite existing log CSV to include Claim column if present; otherwise leave it."""
    if not os.path.exists(run_log):
        return
    try:
        df = pd.read_csv(run_log, dtype=str)
        if "Claim" in df.columns:
            df = enforce_log_cols(df)
            df.to_csv(run_log, index=False)
    except Exception:
        pass

def _pairs_from_log_csv(log_path: str):
    ...
    try:
        ...
        if {"ID", "Claim"}.issubset(df_old.columns):
            ...
            return set((tmp["ID"].str.strip() + "||" + tmp["Claim"].str.lower()).tolist())
        return set()
    except Exception:
        return set()

def _pairs_from_csv(csv_path: str):
    if not os.path.exists(csv_path):
        return set()
    try:
        df_old = pd.read_csv(csv_path, dtype=str)
        if {"ID", "Topic"}.issubset(df_old.columns):
            tmp = df_old[["ID", "Topic"]].fillna("")
            return set((tmp["ID"].str.strip() + "||" + tmp["Topic"].str.lower()).tolist())
        return set()
    except Exception:
        return set()

def save_master_excel(df_sel, out_dir, topic_snake, safe_tag, n_runs, shot_suffix):
    master_xlsx = os.path.join(out_dir, f"{topic_snake}_{safe_tag}_1shot_sheet.xlsx")

    base = df_sel.copy()
    base["ID"] = base["ID"].astype(str).str.strip()
    base["Claim"] = base["Claim"].astype(str)

    wide = pd.DataFrame({
        "ID": base["ID"],
        "Prompt": [build_prompt(t, c) for t, c in zip(base["Text"], base["Claim"])],
        "Topic": base["Claim"],
    })

    csv_dir = os.path.join(out_dir, "csv")
    run_base = f"{topic_snake}_{safe_tag}{shot_suffix}"
...
def run_for_topic(topic_name: str):
    ...
    for idx, r in df.iterrows():
        ...
        sel.append({"ID": str(rid).strip(), "Text": text.strip(), "Claim": claim})
    ...
    out_dir = f"/Users/nonny/Downloads/Senior Project 2025/Task2/Gemma3/Task2_1shot/ABA_task2_{topic_snake}_{SAFE_TAG}{shot_suffix}"
    ...
    for run in range(1, N_RUNS + 1):
        ...
        if RESUME:
            normalize_existing_run_csv(run_csv, run)
            normalize_existing_log_csv(run_log)

        done_pairs = set()
        if RESUME:
            done_pairs |= _pairs_from_csv(run_csv)
            done_pairs |= _pairs_from_log_csv(run_log)

        if PRINT_TO_CONSOLE:
            print(f"\n[{topic_name}] ========= RUN {run} / {N_RUNS} =========")
            if done_pairs:
                print(f"[{topic_name}] Resuming: {len(done_pairs)} (ID+Claim) already completed.")
        ...
        for _, rr in df_sel.iterrows():
            ...
            df_log = pd.DataFrame([{"ID": rid, "Claim": claim, "Output": ans_raw}])
            ...
            if PRINT_TO_CONSOLE:
                print(f"[{topic_name}] [Run {run}] ID={rid} Claim={claim}:\n{ans_raw.strip()}\n")

            if len(rows_batch) >= BATCH_SIZE:
                df_part = pd.DataFrame(rows_batch)
                df_part = enforce_run_cols(df_part, run)
                df_part.to_csv(
                    run_csv,
                    index=False,
                ...
                save_master_excel(df_sel, out_dir, topic_snake, SAFE_TAG, N_RUNS, shot_suffix)
        ...
        try:
            df_full = pd.read_csv(run_csv, dtype=str)
            df_full = enforce_run_cols(df_full, run)
            df_full = df_full.drop_duplicates(subset=["ID","Topic"], keep="last")
    ...
    print(f"[{topic_name}] Saved per-run results to: {os.path.abspath(csv_dir)}")
    print(f"[{topic_name}] Saved per-run LOG CSVs to: {os.path.abspath(log_dir)}")
    print(f"[{topic_name}] Master sheet: {os.path.abspath(os.path.join(out_dir, f'{topic_snake}_{SAFE_TAG}_1shot_sheet.xlsx'))}")

if __name__ == "__main__":
    topics_to_run = TOPICS if RUN_ALL_TOPICS and TOPICS else [TARGET_TOPIC]
